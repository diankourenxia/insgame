...
import { SlideDirection, TileEvent, TileType } from "../type/Enum";
...
export default class TileManager extends cc.Component {
	...
	private eliminateCombinations() {
		ClientEvent.instance().emit("paif_nenglian");
		console.log("要消除的list ", this.combinations)
		for (let i = 0; i < this.combinations.length; i++) {
			console.log("派发 当前消除几个", this.combinations[i].coords.length)
		}
		for (let i = 0; i < this.combinations.length; i++) {
	 ...
	 */
	public static init() {
		this.instance.container.removeAllChildren();
