...
export default class Enemy2 extends cc.Component {
    ...
    set currHp(num: number) {
        ...
        if (num <= 0) {
            let index = Battle2.Ins.enemyNodeList.indexOf(this.node)
            Battle2.Ins.enemyNodeList.splice(index, 1)
            if (Battle2.Ins.enemyNodeList.length <= 0) {
                Battle2.Ins.winNode.active = true
                Battle2.Ins.handler_win()
