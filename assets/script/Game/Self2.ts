...
@ccclass
export default class Self2 extends cc.Component {
    @property(cc.Node)
    ...
    set currHp(num: number) {
        ...
        if (num <= 0) {
            let index = Battle2.Ins.selfNodeList.indexOf(this.node)
            Battle2.Ins.selfNodeList.splice(index, 1)
            if (Battle2.Ins.selfNodeList.length <= 0) {
                Battle2.Ins.loseNode.active = true
                Battle2.Ins.isOver = true
            }
            this.node.destroy()
        }
    }
    ...
    hurt(num) {
        ...
        cc.loader.loadRes(url, cc.Prefab, (err, prefab) => {
            ...
            cc.loader.loadRes(url, cc.Prefab, (err, prefab) => {
                let attackNum = Tools.getRandom(50, 70) + (20 * this.point)
